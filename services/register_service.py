...
def _normalize(raw: dict) -> dict:
    ...
    cfg["always_use_openai_proxy"] = bool(cfg.get("always_use_openai_proxy"))
    cfg["always_use_fetch_remote_info_proxy"] = bool(cfg.get("always_use_fetch_remote_info_proxy"))
...
class RegisterService:
    ...
    def update(self, updates: dict) -> dict:
        with self._lock:
            ...
            openai_register.config.update({k: self._config[k] for k in ("mail", "proxy", "always_use_openai_proxy", "always_use_fetch_remote_info_proxy", "total", "threads")})
            self._save()
            return self.get()
