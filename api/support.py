...
def start_limited_account_watcher(stop_event: Event) -> Thread:
    ...
    def worker() -> None:
        while not stop_event.is_set():
            try:
                all_limited_tokens = account_service.list_limited_tokens()
                limited_tokens = account_service.list_limited_tokens(due_for_refresh=True)
                ...
                if tokens:
                    skipped_limited = len(all_limited_tokens) - len(limited_tokens)
                    print(
                        "[account-watcher] checking "
                        f"{len(limited_tokens)} limited accounts due for refresh"
                        f"{f', skipping {skipped_limited} not yet restored' if skipped_limited else ''}, "
